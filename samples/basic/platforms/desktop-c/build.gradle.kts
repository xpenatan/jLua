...
libfdx {
    desktopC {
        ...
        target("opengl") {
            displayName.set("jLua basic desktop C OpenGL sample")
            mainClass.set("lua.example.basic.LuaBasicDesktopCLauncher")
            targetFileName.set("jlua-basic-desktop-c")
        }
    }
}
...
val integrateJParserTeaVMCResources = tasks.register<Copy>("integrateJParserTeaVMCResources") {
    ...
    doLast {
        ...
        cmakeText = cmakeText
            ...
            .replace(
                "target_link_libraries(jlua-basic-desktop-c PRIVATE glfw3)",
                "target_link_libraries(jlua-basic-desktop-c PRIVATE glfw)"
            )
        ...
        if(!cmakeText.contains(marker)) {
            cmakeText += """
                ...
                $marker
                set(JPARSER_TEAVMC_APP_TARGET jlua-basic-desktop-c)
