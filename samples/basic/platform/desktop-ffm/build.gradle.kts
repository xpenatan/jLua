...
dependencies {
    ...
    implementation(project(":lua:lua-ffm"))
...
tasks.register<JavaExec>("lua_basic_desktop_ffm_run") {
    group = "application"
    description = "Runs the libFDX desktop Lua sample with GL using FFM."
    dependsOn("classes")
    mainClass.set(mainClassName)
    classpath = sourceSets["main"].runtimeClasspath + glRuntimeClasspath
    workingDir = assetsDir
    systemProperty("xlua.sample.backend", "FFM")
    forwardSampleProperty("xlua.sample.exitAfterFrames")
    forwardSampleProperty("xlua.sample.failFast")
